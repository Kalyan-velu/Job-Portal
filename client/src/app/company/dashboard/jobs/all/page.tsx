...
const AllCompanyJobs = memo(() => {
  const { data } = useGetCompanyJobsQuery(undefined, {
        ...
        {data?.map((job) => (
          <JobUpdateDialog job={job}>
            <DialogTrigger asChild>
              <div
                key={job._id}
                className="bg-secondary text-secondary-foreground relative flex min-h-32 w-full cursor-pointer flex-col gap-y-2 rounded-lg p-3 shadow-lg">
